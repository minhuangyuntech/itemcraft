...
- 題庫列表的「瀏覽」會先進入單題瀏覽頁，再由使用者按「編輯」進入編輯狀態。
- 單題瀏覽頁提供「上一題」與「下一題」導覽。
- 導覽範圍可選「不過濾」或「次層面」；選擇「次層面」時只在相同次層面的題目中移動。
- 新增題目時，「拆解 A-D 選項」支援 `A.`、`A、`、`A)`、`A:`、`A：` 等格式。
- 拆解後會把 A-D 選項從題幹移除，並填入四個選項欄位。
- Excel/CSV 匯入器也已同步支援上述選項格式。
- 已清理本機 SQLite 題庫中殘留於題幹的 A-D 選項，並清除選項 D 尾端殘留引號。
- 清理資料前已建立資料庫備份 #5 與 #6；若需在其他環境同步資料，請使用網站匯出功能產生新的資料包。
